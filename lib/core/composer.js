class Composer {
  ...
  filter (test) {
    return this.use(Composer.filter(test))
  }
  ...
  static branch (test, trueMiddleware, falseMiddleware) {
    ...
    return Composer.lazy((ctx) => Promise.resolve(test(ctx))
  ...
  static filter (test) {
    return Composer.branch(test, Composer.passThru(), () => {})
  }
  ...
  static acl (userId, middleware) {
    ...
    if (typeof whitelistFn !== 'function') {
      ...
      whitelistFn = (ctx) => !ctx.from || allowed.includes(ctx.from.id)
    }
