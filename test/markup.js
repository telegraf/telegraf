...
test('should generate keyboard markup', (t) => {
  const markup = {
    ...Markup.keyboard([['one'], ['two', 'three']]).reply_markup,
  }
  t.deepEqual(markup, {
...
test('should generate keyboard markup with options', (t) => {
  const markup = {
    ...Markup.keyboard(['one', 'two', 'three'], { columns: 3 }).reply_markup,
  }
  t.deepEqual(markup, {
    keyboard: [['one', 'two', 'three']],
  })
...
test('should generate keyboard markup with custom columns', (t) => {
  const markup = {
    ...Markup.keyboard(['one', 'two', 'three', 'four'], { columns: 3 })
      .reply_markup,
  }
  t.deepEqual(markup, {
...
test('should generate inline keyboard markup with default setting', (t) => {
  const markup = {
    ...Markup.inlineKeyboard(['one', 'two', 'three', 'four']).reply_markup,
  }
  t.deepEqual(markup, {
    inline_keyboard: [['one', 'two', 'three', 'four']],
  })
})
