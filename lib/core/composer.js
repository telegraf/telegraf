...
class Composer {
  ...
  static entity (predicate, ...fns) {
    ...
    return Composer.optional(({ message }) => {
      return message && message.entities && message.entities.some((entity) => predicate(entity, message.text.substring(entity.offset, entity.offset + entity.length)))
    }, ...fns)
  }
  ...
  static command (command, ...fns) {
    ...
    return Composer.mount('text', Composer.lazy((ctx) => {
      const groupCommands = ctx.me && (ctx.chat.type === 'group' || ctx.chat.type === 'supergroup')
        ...
        : []
      return Composer.entity(({ type }, value) => type === 'bot_command' && (commands.includes(value) || groupCommands.includes(value)), ...fns)
    }))
  }
