class Composer {
  ...
  constructor (...handlers) {
    this.handlers = [Composer.compose(handlers)]
  }
  ...
  use (...fns) {
    ...
    this.handlers.push(Composer.compose(fns))
    return this
  }
  ...
  command (commands, ...fns) {
    ...
    this.use(Composer.command(commands, Composer.compose(fns)))
    return this
  }
  ...
  static branch (test, trueMiddleware, falseMiddleware) {
    return Composer.dispatch(test, {
      true: trueMiddleware,
      false: falseMiddleware
    })
  }
  ...
  static compose (middlewares) {
    if (!Array.isArray(middlewares) || middlewares.find((fn) => !fn || !(typeof fn === 'function' || typeof fn.middleware === 'function'))) {
  ...
  static command (triggers, middleware) {
    ...
    return Composer.mount('text', (ctx, next) => {
      ...
      const hasMatch = ctx.message.entities && ctx.message.entities.find((entity) => {
        ...
        return entity.type === 'bot_command' && (groupCommands.includes(command) || commands.includes(command))
      })
