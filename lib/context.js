...
class TelegrafContext {
   ...
   * @api public
   */
  constructor (telegraf, update, webHookResponse) {
    this.telegraf = telegraf
    this._update = update
    this._state = {}
    if (webHookResponse) {
      this._webHookResponse = webHookResponse
      const old = this.telegraf.sendRequest
      this.telegraf.sendRequest = (method, options) => {
        return old(method, options, webHookResponse)
      }
    }
  }
