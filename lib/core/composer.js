class Composer {
  ...
  static branch (test, trueMiddleware, falseMiddleware) {
    ...
    return (ctx, next) => Promise.resolve(test(ctx))
      .then((value) => {
        ...
        return unwrap(handler)(ctx, next)
      })
  }
  ...
  static dispatch (test, handlers) {
    ...
    return (ctx, next) => Promise.resolve(test(ctx))
      .then((value) => {
        const handler = unwrap(handlers[value]) || Composer.passThru()
  ...
  static match (tests, middleware) {
    return (ctx, next) => {
      ...
      for (let test of tests) {
        ...
        ctx.match = result
        return unwrap(middleware)(ctx, next)
      }
      return next()
    }
  }
  ...
  static command (command, middleware) {
    ...
    return Composer.mount('text', (ctx, next) => {
      ...
      return hasMatch ? unwrap(middleware)(ctx, next) : next()
    })
  }
