...
class Telegram extends ApiClient {
  ...
  getFileLink (file) {
    const prefix = `${this.options.apiRoot}/file/bot${this.token}`
    if (file && file.file_path) {
      return `${prefix}/${file.file_path}`
    }
    const fileId = file && file.file_id ? file.file_id : file
    return this.getFile(fileId).then((file) => `${prefix}/${file.file_path}`)
  }
