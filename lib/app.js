...
telegraf.startPolling = function () {
  this.started = true
  this.updateLoop()
  return this
}
...
telegraf.on = function (messageType) {
  ...
  this.bus.on(messageType, function (msg) {
    ...
    context.context.reply = function (message, options) {
      return this.sendMessage(msg.chat.id, message, options)
    }.bind(this)

    shortcuts.forEach(function (methodName) {
      context.context[methodName.replace('send', 'replyWith')] = function (obj, options, extra) {
...
telegraf.answerCallbackQuery = function (callbackQueryId, text, show_alert) {
  var options = {
