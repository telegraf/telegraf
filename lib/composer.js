class Composer {
  ...
  static tap (fn) {
    return (ctx, next) => {
      try {
        ...
        console.error('Telegraf: error in `tap` handler', err)
      }
      return next()
    }
  }
  ...
  static mount (updateTypes, middleware) {
    ...
    return Composer.optional((ctx) => updateTypes.indexOf(ctx.updateType) !== -1 || updateTypes.indexOf(ctx.updateSubType) !== -1, middleware)
  }
