...
export interface ContextMessageUpdate extends Context {
   ...
   *
   * @returns Exported invite link as a string on success.
   */
  exportChatInviteLink(): Promise<string>

  /**
   * Use this method to get up to date information about the chat (current
   * name of the user for one-on-one conversations, current username of a user,
   * group or channel, etc.)
   *
  ...
  /**
   * Use this method to get a list of administrators in a chat. An array, which
   * contains information about all chat administrators, except bots. If the
   * chat is a group or a supergroup and no administrators were appointed, only
  ...
  /**
   * Use this method to get the count of members in a chat
   *
  ...
  /**
   * Use this method to send audio files to the same chat, if you want
   * Telegram clients to display them in the music player. Your audio must be
   * in the .mp3 format. Bots can currently send audio files of up to 50 MB
   * in size, this limit may be changed in the future.
   *
   * @param audio Audio file to be sent.
   * - Pass a file_id as a String for send an audio file which exists on
   ...
   *
   * @param document File to be sent
   * - Pass the id of a file as a String to send a file which exists on the
   * Telegram servers (recommended)
   * - Pass the URL as a String for Telegram to get a file from the Internet,
   ...
   *
   * @param html You can use bold and italic text, as well as inline links
   ...
   * The Bot API supports basic formatting for messages.
   *
   * @param markdown You can use bold and italic text, as well as inline
   * links and pre-formatted code in your bots' messages.
   * @param extra Additional params to send message
   * @returns A Message on success
   */
  replyWithMarkdown(
   ...
   * Telegram servers (recommended)
   * - Pass an HTTP URL as a String for Telegram to get a .webp file from
   ...
   * (other formats may be sent as a Document). Bots can currently send video
   ...
   * @param cacheTime The maximum amount of time in seconds that the result
   * of the callback query may be cached client-side. Telegram apps will
   ...
   * @param shippingOptions Required if ok is True. A JSON-serialized array
   ...
   * and the bot is ready to proceed with the order. Use False if there are any
   * problems.
   * @param errorMessage Required if ok is False. Error message in human
   ...
   * @returns If edited message is sent by the bot, the edited Message will be
   ...
   * times). https://core.telegram.org/bots/api#sending-files
   *
