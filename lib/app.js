...
telegraf.handleUpdate = function (update) {
  var currentTypes = []
  var payload
  supportedUpdateTypes.forEach(function (key) {
    if (update[key]) {
      payload = update[key]
  ...
  var tasks = this.handlers
    .filter(function (handler) {
      return handler.eventTypes.filter(function (n) { return currentTypes.indexOf(n) !== -1 }).length > 0
    })
    .map(function (handler) {
      var context = self.createContext(currentTypes[currentTypes.length - 1], payload)
      context.use(compose(self.middleware))
      context.use(compose(handler.middleware))
...
telegraf.createContext = function (payloadType, payload) {
  debug('Create context', payloadType)
  var state = {}
