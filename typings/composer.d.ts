...
export type Middleware<TContext extends TelegrafContext> = MiddlewareFn<TContext> | MiddlewareObj<TContext>;

export declare class Composer<TContext extends TelegrafContext> implements MiddlewareObj<TContext> {
	/**
   ...
   */
	hears(triggers: HearsTriggers<TContext>, ...middlewares: ReadonlyArray<Middleware<TContext>>): this;

	/**
   * Registers middleware for handling callbackQuery data with regular expressions
   */
	action(triggers: HearsTriggers<TContext>, ...middlewares: ReadonlyArray<Middleware<TContext>>): this;

	/**
   * Registers middleware for handling inlineQuery messages.
   */
	inlineQuery(triggrers: HearsTriggers<TContext>, ...middlewares: ReadonlyArray<Middleware<TContext>>): this;

	/**
   * Registers middleware for handling specified commands.
   */
	command(command: string | string[], ...middlewares: ReadonlyArray<Middleware<TContext>>): this;
   ...
   */
	...
	static unwrap<TContext extends TelegrafContext>(middleware: Middleware<TContext>): MiddlewareFn<TContext>;
   ...
   */
	static filter<TContext extends TelegrafContext>(predicate: BranchPredicate<TContext>): MiddlewareFn<TContext>;

	/**
   * Generates drop middleware.
   */
	static drop<TContext extends TelegrafContext>(predicate: BranchPredicate<TContext>): Middleware<TContext>;
   ...
   */
	...
	static reply(text: string, extra?: tt.ExtraReplyMessage): MiddlewareFn<TelegrafContext>;

	/**
   * Generates middleware that runs in the background.
   */
	static fork<TContext extends TelegrafContext>(middleware: Middleware<TContext>): MiddlewareFn<TContext>;
