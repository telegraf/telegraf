...
telegraf.sendRequest = function (method, options, response) {
  ...
  var isMultipartRequest = Object.keys(options).filter((x) => options[x] && options[x].source).length > 0
  if (response && !response.finished && !isMultipartRequest && constants.webHookAnswerBlacklist.indexOf(method) === -1) {
  ...
  debug('▶︎ http', method)
  var buildBody = isMultipartRequest
  ...
  return buildBody
    .then((body) => {
      var payload = {
        method: 'POST',
        headers: isMultipartRequest ? {} : {'Content-Type': 'application/json'},
        body: body
      }
