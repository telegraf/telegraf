...
export interface ContextMessageUpdate extends Context {
   ...
   * @returns On success, returns an Array of ChatMember objects that contains
   * an information about all chat administrators except other bots. If the chat
   ...
   * @param extra Pass `{ disable_notification: true }`, if it is not necessary
  ...
  /**
   * Use this method to send audio files to the same chat, if you want Telegram
   ...
   * @param html You can use bold and italic text, as well as inline links and
   * pre-formatted code in your bots' messages.
  ...
  /**
   * The Bot API supports basic formatting for messages
   * @param markdown You can use bold and italic text, as well as inline links
   ...
   * String for Telegram to get a .webp file from the Internet, or upload a new
   ...
   * exists on the Telegram servers (recommended), pass an HTTP URL as a String
   * for Telegram to get a video from the Internet, or upload a new video using
   * multipart/form-data
   ...
   * (e.g. "Sorry, delivery to your desired address is unavailable'). Telegram
   ...
   * choose a different color or garment!"). Telegram will display this message
   * to the user.
   */
   ...
   * @param stickerFile Png image with the sticker, must be up to 512 kilobytes
   * in size, dimensions must not exceed 512px, and either width or height must
   * be exactly 512px.
...
export interface Telegram {
   ...
   * bot (for inline bots). On success, if edited message is sent by the bot,
   * the edited Message is returned, otherwise True is returned.
   * @param chatId Required if inlineMessageId is not specified. Unique
   * identifier for the target chat or username of the target channel (in the
   * format @channelusername)
   * @param messageId Required if inlineMessageId is not specified. Identifier
   * of the sent message
   * @param inlineMessageId Required if chatId and messageId are not specified.
   ...
   * webhook integration
   * @param cert Upload your public key certificate so that the root certificate
   * in use can be checked
   * @param maxConnections Maximum allowed number of simultaneous HTTPS
...
export interface Composer<TContext extends ContextMessageUpdate> {
   ...
   */
  on(
    updateTypes: tt.UpdateType | tt.UpdateType[] | tt.MessageSubTypes | tt.MessageSubTypes[],
    middleware: Middleware<TContext>,
    ...middlewares: Array<Middleware<TContext>>
  ): Composer<TContext>
...
export interface ComposerConstructor {
  ...
  /**
   * Compose middlewares returning a fully valid middleware comprised of all
   ...
   * @param middleware Middleware function
   */
  mount<TContext extends ContextMessageUpdate,
   ...
   */
  hears<TContext extends ContextMessageUpdate,
  UContext extends ContextMessageUpdate>(
    triggers: HearsTriggers,
  ...
  /**
   * Generates middleware for handling callbackQuery data with regular
   * expressions.
   * @param triggers Triggers
   * @param handler Handler
   */
  action<TContext extends ContextMessageUpdate,
  UContext extends ContextMessageUpdate>(
    triggers: HearsTriggers,
    handler: Middleware<TContext>
  ): Middleware<UContext>
   ...
   * @param middleware Middleware function
   */
  optional<TContext extends ContextMessageUpdate,
  UContext extends ContextMessageUpdate>(
   ...
   */
  branch<TContext extends ContextMessageUpdate,
  UContext extends ContextMessageUpdate,
  VContext extends ContextMessageUpdate,
  WContext extends ContextMessageUpdate>(
    test: boolean | ((ctx: TContext) => boolean),
    trueMiddleware: Middleware<UContext>,
    falseMiddleware: Middleware<VContext>
  ): Middleware<WContext>
...
export interface Telegraf<TContext extends ContextMessageUpdate> extends Composer<TContext> {
  ...
  /**
   * Return a callback function suitable for the http[s].createServer() method
   * to handle a request. You may also use this callback function to mount your
   * telegraf app in a Koa/Connect/Express app.
   * @param webhookPath Webhook url path (see Telegraf.setWebhook)
   */
  webhookCallback(webhookPath: string): (
    req: IncomingMessage,
    res: ServerResponse
  ) => void

  /**
   * Handle raw Telegram update. In case you use centralized webhook server,
   * queue, etc.
